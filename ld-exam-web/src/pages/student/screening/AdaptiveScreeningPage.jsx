...
export default function AdaptiveScreeningPage() {
  ...
  const rootStyle = {
    ...
    background: a11y.highContrast ? '#000' : '#F5F8FD',
    ...
    <div style={rootStyle}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: a11y.highContrast ? '#fff' : '#1E4FA0', margin: 0, letterSpacing: '-0.01em' }}>English + Math Skill Assessment</h1>

          <p style={{ fontSize: 13, color: a11y.highContrast ? '#ccc' : '#64748b', margin: '4px 0 0' }}>Measures your ability across English and Mathematics skills.</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {SUBJECT_TABS.map((s) => {
              ...
              <span key={s} style={{
                ...
                background: isActive ? (isMath ? '#0EA5A4' : '#2563EB') : (a11y.highContrast ? '#222' : '#EAF1FB'),

                color: isActive ? '#fff' : (a11y.highContrast ? '#ddd' : '#33507e') }}>

                <span aria-hidden="true">{isMath ? '➕' : '📘'}</span>{s}</span>
          ...
          <button onClick={() => setShowA11y((v) => !v)}
            ...
            style={{ border: 'none', background: '#2563EB', color: '#fff', borderRadius: 999,

              padding: '8px 16px', fontWeight: 700, cursor: 'pointer', minHeight: 40,
        ...
        <div style={{ height: 10, background: a11y.highContrast ? '#333' : '#E2EAF6', borderRadius: 999, overflow: 'hidden', marginBottom: 6 }}>

          <div style={{ width: `${progress}%`, height: '100%', background: 'linear-gradient(90deg, #2563EB, #0EA5A4)', transition: 'width .3s' }} />
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 700,
            ...
            background: a11y.highContrast ? '#222' : (q.subject === 'Mathematics' ? '#E3F6F5' : '#EAF1FB'),

            color: a11y.highContrast ? '#ddd' : (q.subject === 'Mathematics' ? '#0EA5A4' : '#2563EB') }}>{q.skill}</span>

          <span style={{ padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 700,

            background: a11y.highContrast ? '#222' : '#F1F5F9', color: a11y.highContrast ? '#ddd' : '#64748b' }}>{q.subSkill}</span>

          <span style={{ padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 700,

            background: a11y.highContrast ? '#222' : (q.difficulty === 'Advanced' ? '#FDECEC' : q.difficulty === 'Intermediate' ? '#FEF3E2' : '#E7F6EC'),

            color: a11y.highContrast ? '#ddd' : (q.difficulty === 'Advanced' ? '#EF4444' : q.difficulty === 'Intermediate' ? '#F59E0B' : '#16A34A') }}>{q.difficulty}</span>

        </div>

        {/* Question card */}

        <div style={{ background: cardBg, borderRadius: 20, padding: 28, boxShadow: '0 10px 30px rgba(15,60,107,0.10)', border: `1px solid ${a11y.highContrast ? '#333' : '#E2E8F0'}` }}>

          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>

            <p style={{ fontSize: '1.3em', fontWeight: 700, margin: 0, flex: 1, lineHeight: 1.5 }}>{q.question}</p>
...
function navBtn(primary, disabled) {
  ...
  return {
    ...
    padding: '12px 26px', borderRadius: 999, fontWeight: 700, border: 'none',

    cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1,

    background: primary ? '#2563EB' : '#E2E8F0', color: primary ? '#fff' : '#334155',

    boxShadow: primary && !disabled ? '0 6px 16px rgba(37,99,235,0.30)' : 'none',
