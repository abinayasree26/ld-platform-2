    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main className="sp-main" style={{ flex: 1, marginLeft: collapsed ? 0 : 220, transition: 'margin-left 0.2s ease', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div className="sp-content sp-flexrow" style={{ flex: 1, minHeight: 0, overflow: 'hidden', padding: 32, display: 'flex', flexDirection: 'column' }}>
          <button
            onClick={() => navigate(-1)}
            style={{ flexShrink: 0, alignSelf: 'flex-start', background: '#fff', border: '1px solid #e2e8f0', color: '#475569', fontSize: 11, fontWeight: 700, cursor: 'pointer', padding: '5px 11px', borderRadius: 8, marginBottom: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}
          >
            Back
          </button>
          <h2 style={{ flexShrink: 0, fontSize: 20, fontWeight: 800, color: '#1e293b', margin: '0 0 4px' }}>🆘 Help &amp; Support</h2>
          ...
          <div style={{ ...card, padding: 0, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
            ...
            <div style={{ flexShrink: 0, padding: '10px 20px', borderTop: '1px solid #f1f5f9', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {FAQS.map((f) => (
                <button
                  key={f.q}
                  onClick={() => askFaq(f)}
