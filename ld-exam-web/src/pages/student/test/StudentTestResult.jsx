      ...
      <div className={`rounded-2xl border-2 bg-gradient-to-br ${heroBg} p-8 text-center mb-6`}>
        ...
        {leveledUp && (
          <div className="mt-5 flex flex-col items-center gap-2">
            <div style={{ animation: 'pop-bounce 0.5s ease-out' }}>
              <LevelAvatar level={level + 1} size={84} />
            </div>
            <div className="bg-green-600 text-white rounded-xl px-5 py-2.5 inline-block font-bold text-sm">
              🔓 New Avatar Unlocked! Level {level + 1} ({LEVEL_LABELS[level + 1]})
            </div>
            <style>{`@keyframes pop-bounce { 0% { transform: scale(0) rotate(-15deg); opacity: 0; } 60% { transform: scale(1.15) rotate(6deg); opacity: 1; } 100% { transform: scale(1) rotate(0deg); } }`}</style>
          </div>
        )}
      </div>
