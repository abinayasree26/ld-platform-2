...
-- fee_concessions already created in 003_school_schema.sql (without these columns) -- extend it
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS fee_head_id UUID REFERENCES fee_heads(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS fee_structure_id UUID REFERENCES fee_structures(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS concession_type VARCHAR(30);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS percentage DECIMAL(5,2);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS calculated_amount DECIMAL(10,2);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS academic_year_id UUID REFERENCES academic_years(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS applicable_from DATE;
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS applicable_to DATE;
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'pending';
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS requested_by UUID REFERENCES users(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS approved_amount DECIMAL(10,2);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS approved_by UUID REFERENCES users(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS approved_at TIMESTAMP;
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS approval_notes TEXT;
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS rejection_reason TEXT;
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS rejected_by UUID REFERENCES users(id);
ALTER TABLE fee_concessions ADD COLUMN IF NOT EXISTS rejected_at TIMESTAMP;
